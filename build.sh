...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e freqfilter.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/freqfilter-1.0.0
        tar czf ${tmpdir}/freqfilter-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} freqfilter-1.0.0
        rpmbuild -ta ${tmpdir}/freqfilter-1.0.0.tar.gz
        rm -rf $tmpdir
    else
